...
_Bool dijkstra_set_graph(int e, int edges[][3], void *dijkstra)
{
   ...
   if (dij)
   {
      ...
      for (int i = 0; i < e; ++i)
      {
         x = x < edges[i][0] ? edges[i][0] : x;
         x = x < edges[i][1] ? edges[i][1] : x;
      }
      x += 1;

      dij->nodes = myMalloc(sizeof(node_t) * x);
      if (dij->nodes)
      {
         dij->num_nodes = x;
   ...
   if (dij)
   {
      ...
      if (f)
      {
         for (int i = 0; i < dij->num_nodes; ++i)
         {
            ...
            write_int(f, i);
            putc(' ', f);
            write_int(f, node->cost);
            putc(' ', f);
            write_int(f, node->parent);
            putc('\n', f);
         } // end all nodes
         ret = fclose(f) == 0;
      }
