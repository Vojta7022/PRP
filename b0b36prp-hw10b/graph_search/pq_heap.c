...
void pq_heapify_up(pq_heap_s *pq, int idx);
void pq_heapify_down(pq_heap_s *pq, int idx);
...
void pq_heapify_up(pq_heap_s *pq, int idx)
{
   ...
   {
      int parent = (idx - 1) / 2;
      if (pq->cost[idx] < pq->cost[parent])
      {
         // Swap the elements
         int temp_cost = pq->cost[idx];
         pq->cost[idx] = pq->cost[parent];
         pq->cost[parent] = temp_cost;

         int temp_label = pq->label[idx];
         pq->label[idx] = pq->label[parent];
         pq->label[parent] = temp_label;

         pq->heapIDX[pq->label[idx]] = idx;
         pq->heapIDX[pq->label[parent]] = parent;
...
void pq_heapify_down(pq_heap_s *pq, int idx)
{
   ...
   while ((left = 2 * idx + 1) < pq->len)
   {
      ...
      if (smallest != idx)
      {
         // Swap the elements
         int temp_cost = pq->cost[idx];
         pq->cost[idx] = pq->cost[smallest];
         pq->cost[smallest] = temp_cost;

         int temp_label = pq->label[idx];
         pq->label[idx] = pq->label[smallest];
         pq->label[smallest] = temp_label;

         pq->heapIDX[pq->label[idx]] = idx;
         pq->heapIDX[pq->label[smallest]] = smallest;
...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (pq && pq->len < pq->size && label >= 0 && label < pq->size && pq->heapIDX[label] != -1) // vertex with the label is in the pq
   {
      ...
      int old_cost = pq->cost[idx];
      pq->cost[idx] = cost; // update the cost

      if (cost < old_cost)
      {
         pq_heapify_up(pq, idx); // move the updated element up if the cost decreased
      }
      else
      {
         pq_heapify_down(pq, idx); // move the updated element down if the cost increased
      }
