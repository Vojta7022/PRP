...
int main(int argc, char *argv[])
{
  ...
  for (size_t i = 0; i < MAX_SEQUENCE; i++)
  {
    ...
    if (read_matrix(&matrices[index], input) == ERROR_INPUT)
    {
      ...
      for (size_t i = 0; i < MAX_SEQUENCE; i++)
      {
        // printf("Freed %d elements in main - read matrix failed - ", MAX_ELEMENTS);
  ...
  if (compute_sequence(matrix_sequence, operation_sequence, &matrix_count, &operation_count) == ERROR_INPUT)
  {
    ...
    {
      // printf("Freed %d elements in main - compute sequence failed - ", MAX_ELEMENTS);
      // print_matrix(matrix_sequence[i]);
  ...
  // printf("Freed %ld elements in main - end\n", matrix_sequence[0].size);
  // print_matrix(matrix_sequence[0]);
  ...
  for (size_t i = 0; i < MAX_SEQUENCE; i++)
  {
    if (matrices[i].cols > 0)
    {
      // printf("Freed %ld elements in main - end - matrices - %ld", sizeof(matrices[i]) / sizeof(int), i);
      // print_matrix(matrices[i]);
      free_matrix(&matrices[i]);
    }
  }
...
int read_matrix(matrix_t *matrix, char input[])
{
  ...
  // printf("Allocated %d elements in read_matrix - \n", INIT_SIZE);
  ...
  while (input[i] != '\n' && input[i] != '\0')
  {
    ...
    if (elems >= matrix->size)
    {
      ...
      matrix->size = 2 * elems;
      // printf("Reallocated %d elements in read_matrix - \n", 2 * elems);
    }
  }
...
int mul_matrix(matrix_t *matrix1, matrix_t *matrix2)
{
  ...
  // printf("Allocated %d elements in mul_matrix - ", result_matrix.rows * result_matrix.cols);
...
int compute_sequence(matrix_t matrices[], operation_t operations[], int *matrix_count, int *operation_count)
{
  while (*operation_count > 0)
  {
    ...
    // printf("Freed %ld elements in compute_sequence - ", matrix2->size);
    // print_matrix(*matrix2);
    free_matrix(matrix2);
